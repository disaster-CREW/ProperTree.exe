...
try:
    import sv_ttk as _SV_TTK  # Sun Valley ttk theme (optional)
except ImportError:
    _SV_TTK = None

def _check_for_update(queue, version_url = None, user_initiated = False):
    if getattr(sys, 'frozen', False):
        return queue.put({"error":"Update check is not available in the packaged build.","user_initiated":user_initiated})
    args = [sys.executable]
...
def _update_tex(queue, tex_url = None, tex_path = None):
    if getattr(sys, 'frozen', False):
        return queue.put({"error":"Configuration.tex download is not available in the packaged build."})
    args = [sys.executable]
    file_path = os.path.join(os.path.abspath(os.path.dirname(__file__)),"Scripts","update_check.py")
    if os.path.exists(file_path):
...
def find_icon(filename):
    """Locate an ico file whether running from source or a PyInstaller bundle."""
    candidates = []
    if hasattr(sys, '_MEIPASS'):
        candidates.append(os.path.join(sys._MEIPASS, 'Scripts', filename))
    exe_dir = os.path.abspath(os.path.dirname(sys.executable if getattr(sys, 'frozen', False) else __file__))
    candidates.append(os.path.join(exe_dir, '_internal', 'Scripts', filename))
    candidates.append(os.path.join(exe_dir, 'Scripts', filename))
...
def set_window_icons(window, taskbar_ico, corner_ico):
    """Set ProperTree.ico on the taskbar and title_window.ico on the title bar corner."""
    ...
    try:
        ...
        if taskbar_ico:
            hicon_big = ctypes.windll.user32.LoadImageW(
                None, taskbar_ico, IMAGE_ICON,
                0, 0, LR_LOADFROMFILE | LR_DEFAULTSIZE)
            ctypes.windll.user32.SendMessageW(hwnd, WM_SETICON, ICON_BIG, hicon_big)
        if corner_ico:
            hicon_small = ctypes.windll.user32.LoadImageW(
                None, corner_ico, IMAGE_ICON,
...
class ProperTree:
    def __init__(self, plists = []):
        ...
        # Prefer system UI fonts on Windows so the app matches native / editor-style apps
        if sys.platform == "win32" and os.name == "nt":
            for _pat, _font in (
                ("*TkDefaultFont", "{Segoe UI} 10"),
                ("*TkTextFont", "{Segoe UI} 10"),
                ("*TkMenuFont", "{Segoe UI} 10"),
                ("*TkHeadingFont", "{Segoe UI} 10"),
                ("*TkCaptionFont", "{Segoe UI} 11"),
            ):
                try:
                    self.tk.option_add(_pat, _font, "widgetDefault")
                except Exception:
                    pass
        # Set taskbar and window icons on Windows
        if sys.platform == 'win32':
            try:
                ctypes.windll.shell32.SetCurrentProcessExplicitAppUserModelID(u'CorpNewt.ProperTree')
            except Exception:
                pass
            self._taskbar_ico = find_icon('ProperTree.ico')
            self._corner_ico  = find_icon('title_window.ico')
        else:
            self._taskbar_ico = None
            self._corner_ico  = None
        ...
        # Auto-save settings
        sep_autosave = ttk.Separator(self.settings_window,orient="horizontal")
        sep_autosave.grid(row=21,column=0,columnspan=7,sticky="we",padx=10,pady=(5,0))
        self.auto_save_int = tk.IntVar()
        self.auto_save_check = tk_or_ttk.Checkbutton(self.settings_window,text="Enable Auto-Save",variable=self.auto_save_int,command=self.auto_save_command)
        self.auto_save_check.grid(row=22,column=0,columnspan=2,sticky="w",padx=10,pady=(5,0))
        as_interval_label = tk.Label(self.settings_window,text="Interval (seconds):")
        as_interval_label.grid(row=23,column=0,sticky="w",padx=10,pady=(5,0))
        self.auto_save_interval = tk.StringVar(value="60")
        self.auto_save_spinbox = tk.Spinbox(self.settings_window,from_=10,to=3600,textvariable=self.auto_save_interval,width=6,command=self.auto_save_interval_command)
        self.auto_save_spinbox.grid(row=23,column=1,sticky="w",padx=10,pady=(5,0))
        as_folder_label = tk.Label(self.settings_window,text="Auto-Save Folder (leave blank to save in place):")
        as_folder_label.grid(row=24,column=0,columnspan=4,sticky="w",padx=10,pady=(5,0))
        self.auto_save_folder = tk.StringVar()
        self.auto_save_folder_entry = tk_or_ttk.Entry(self.settings_window,textvariable=self.auto_save_folder)
        self.auto_save_folder_entry.grid(row=25,column=0,columnspan=5,sticky="we",padx=10,pady=(0,5))
        self.auto_save_browse = tk_or_ttk.Button(self.settings_window,text="Browse...",command=self.browse_auto_save_folder)
        self.auto_save_browse.grid(row=25,column=5,columnspan=2,sticky="we",padx=10,pady=(0,5))
        ...
        # Defaults tuned toward VS Code–style tree / editor surfaces (user settings.json overrides)
        self.default_dark  = {
            "alternating_color_1":"#1e1e1e",
            "alternating_color_2":"#252526",
        ...
        self.default_light = {
            "alternating_color_1":"#ffffff",
            "alternating_color_2":"#f3f3f3",
            "highlight_color":"#cce8ff",
        ...
        key="Control"
        sign = "Ctrl+"
        self._sv_ttk = _SV_TTK
        self.use_dark = self.get_dark()
        self._refresh_modern_theme()
        ...
        # Set our titlebar colors and corner/taskbar icons (convert + settings windows)
        for w in (self.tk, self.settings_window):
            self.set_win_titlebar(windows=w)
            self.schedule_window_icons(w)
    ...
    def check_dark_mode(self):
        ...
        if check_dark != self.use_dark:
            if any((x not in self.settings for x in ("alternating_color_1","alternating_color_2","background_color"))):
                # Update plist/tree colors when using built-in defaults
    ...
    def auto_save_loop(self):
        if self.settings.get("auto_save_enabled", False):
            try:
                save_folder = self.settings.get("auto_save_folder", "").strip()
                for window in self.stackorder(self.tk):
                    if window in self.default_windows:
                        continue
                    if window.edited and window.current_plist:
                        if save_folder and os.path.isdir(save_folder):
                            import shutil
                            dest = os.path.join(save_folder, os.path.basename(window.current_plist))
                            if os.path.exists(window.current_plist):
                                shutil.copy2(window.current_plist, dest)
                            window.save_plist()
                        else:
                            window.save_plist()
            except Exception:
                pass
        # Schedule next run based on user interval
        try:
            interval = max(10, int(self.settings.get("auto_save_interval", 60))) * 1000
        except:
            interval = 60000
        self.tk.after(interval, self.auto_save_loop)

    def schedule_window_icons(self, window):
        """Defer WM_SETICON for a Toplevel/Tk (Windows); no-op elsewhere or if no .ico paths."""
        if sys.platform != "win32":
            return
        if not (self._taskbar_ico or self._corner_ico):
            return
        try:
            window.after(
                150,
                lambda w=window, t=self._taskbar_ico, c=self._corner_ico: set_window_icons(w, t, c),
            )
        except Exception:
            pass

    def _modern_palette(self):
        """VS Code–like shell colors for tk surfaces (not plist tree cells)."""
        if self.use_dark:
            return {"shell": "#1e1e1e", "panel": "#252526", "fg": "#cccccc"}
        return {"shell": "#f3f3f3", "panel": "#ececec", "fg": "#1e1e1e"}

    def _apply_sun_valley_theme(self):
        if self._sv_ttk is None:
            return
        try:
            self._sv_ttk.set_theme("dark" if self.use_dark else "light")
        except Exception:
            pass

    def _apply_modern_shell_colors(self):
        """Apply tk option database so classic tk widgets match Sun Valley / VS Code chrome."""
        pal = self._modern_palette()
        shell, panel, fg = pal["shell"], pal["panel"], pal["fg"]
        pri = 80  # interactive priority so theme wins over widget defaults
        try:
            for pat, val in (
                ("*Toplevel.background", shell),
                ("*Frame.background", panel),
                ("*Label.background", panel),
                ("*Label.foreground", fg),
                ("*Spinbox.background", panel),
                ("*Spinbox.foreground", fg),
                ("*Spinbox.insertBackground", fg),
            ):
                self.tk.option_add(pat, val, pri)
        except Exception:
            pass
        try:
            self.tk.configure(background=shell)
            self.settings_window.configure(background=shell)
        except Exception:
            pass
        skip = frozenset(getattr(self, "canvas_connect", {}) or ())
        try:
            self._patch_shell_widgets_under(self.tk, shell, panel, fg, skip)
            self._patch_shell_widgets_under(self.settings_window, shell, panel, fg, skip)
        except Exception:
            pass

    def _patch_shell_widgets_under(self, root, shell, panel, fg, skip=frozenset()):
        """Recolor existing tk Frame/Label/Spinbox under root (skip color-picker canvases)."""
        for child in root.winfo_children():
            ...
            if cls == "Frame":
                try:
                    child.configure(background=panel, highlightthickness=0)
                except Exception:
                    pass
            elif cls == "Label":
                try:
                ...
                try:
                    child.configure(background=panel, foreground=fg, insertbackground=fg)
                except Exception:
                    pass
            try:
                self._patch_shell_widgets_under(child, shell, panel, fg, skip)
            except Exception:
                pass

    def _sync_all_plist_shells(self):
        for w in self.stackorder(self.tk, include_defaults=True):
            if w in getattr(self, "default_windows", ()):
                continue
            fn = getattr(w, "sync_modern_shell", None)
            if callable(fn):
                try:
                    fn()
                except Exception:
    ...
    def _refresh_modern_theme(self):
        """Sun Valley + tk shell + plist chrome; safe if sv_ttk missing."""
        self._apply_sun_valley_theme()
        self._apply_global_ttk_chrome()
        self._apply_modern_shell_colors()
        self._sync_all_plist_shells()

    def _apply_global_ttk_chrome(self):
        """Light ttk tweaks; do not override Sun Valley theme when sv_ttk is active."""
        try:
            s = ttk.Style(self.tk)
            if self._sv_ttk is None:
                if sys.platform == "win32" and os.name == "nt":
                ...
                s.configure("TCheckbutton", padding=(2, 5))
                s.configure("TMenubutton", padding=(8, 5))
                s.configure("TEntry", padding=(5, 3))
                s.configure("TSpinbox", padding=(4, 2))
                s.configure("TScale", sliderthickness=16)
                s.configure("TScrollbar", arrowsize=12)
            else:
                # Sun Valley already styles controls; keep only subtle spacing where it helps
    ...
    def set_win_titlebar(self, windows=None, mode=None):
        ...
        try:
            ...
            # Windows 11: rounded corners on the window frame (ignored on older OS)
        ...
        for window in windows:
            try:
                ...
                set_window_attribute(hwnd_inst, DWMWA_USE_IMMERSIVE_DARK_MODE_BEFORE_20H1, ctypes.byref(value),
                                    ctypes.sizeof(value))
                # No-op on Windows 10 / unsupported DWM (returns E_INVALIDARG)
    ...
    def auto_save_command(self, event = None):
        self.settings["auto_save_enabled"] = True if self.auto_save_int.get() else False

    def auto_save_interval_command(self, event = None):
        try:
            val = int(self.auto_save_interval.get())
            if val < 10: val = 10
            self.settings["auto_save_interval"] = val
        except:
            pass

    def browse_auto_save_folder(self, event = None):
        folder = fd.askdirectory(title="Select Auto-Save Folder")
        if folder:
            self.auto_save_folder.set(folder)
            self.settings["auto_save_folder"] = folder
    ...
    def update_settings(self):
        ...
        self.auto_save_int.set(self.settings.get("auto_save_enabled", False))
        self.auto_save_interval.set(str(self.settings.get("auto_save_interval", 60)))
        self.auto_save_folder.set(self.settings.get("auto_save_folder", ""))
        self._refresh_modern_theme()
    ...
    def duplicate_plist(self, event = None):
        ...
        self.schedule_window_icons(new_window)
    ...
    def new_plist(self, event = None):
        ...
        self.schedule_window_icons(window)
        # Update the Open Recent menu
    ...
    def open_plist_with_path(self, event = None, path = None, current_window = None):
        ...
        if not current_window:
            ...
            self.schedule_window_icons(current_window)
        # Ensure the window titlebar color is updated
...
if __name__ == '__main__':
    ...
    # When running as a frozen PyInstaller exe, swap multiprocessing
    # for threading to avoid Windows spawn crashes, and suppress
    # console windows from subprocess calls.
    if getattr(sys, 'frozen', False):
        import threading
